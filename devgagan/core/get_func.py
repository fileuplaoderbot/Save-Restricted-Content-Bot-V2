        ...
        if 't.me/c/' in msg_link or 't.me/b/' in msg_link:
            ...
            if chat in saved_channel_ids:
                await app.edit_message_text(
                    ...
                    "Sorry! This channel is protected by **__Shinchan Dubber__**."
                )
                return
...
get_user_rename_preference = lambda user_id: user_rename_preferences.get(str(user_id), 'Shinchan Dubber')
...
async def send_settings_message(chat_id, user_id):
    ...
    buttons = [
        ...
        [Button.url("Report Errors", "https://t.me/MovieHorn_ContactRobot")]
    ]
...
async def callback_query_handler(event):
    ...
    if event.data == b'setchat':
        ...
        await event.respond("Watermark is Pro+ Plan.. contact @MovieHorn_ContactRobot")
        return
        ...
        await event.edit("Choose your preferred upload method:\n\n__**Note:** **SpyLib ⚡**, built on Telethon(base), by Shinchan Dubber still in beta.__", buttons=buttons)
    ...
    try:
        ...
        if freecheck == 1:
            reply_markup = InlineKeyboardMarkup(
                [
                    [InlineKeyboardButton("💎 Get Premium to Forward", url="https://t.me/MovieHorn_ContactRobot")]
                ]
            )
...
def progress_callback(done, total, user_id):
    ...
    final = (
        ...
        f"╰──────────────────╯\n\n"
        f"**__Powered by Shinchan Dubber__**"
    )
